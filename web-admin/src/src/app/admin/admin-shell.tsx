...
export default function AdminShell({
  ...
  return (
    <div className="flex min-h-screen min-h-[100dvh] bg-[#F7F9F8] text-slate-900 font-sans antialiased selection:bg-emerald-100 selection:text-emerald-900">
      {/* ─── Desktop Sidebar ─── */}
      <aside
          ...
          <div className="fixed inset-y-0 left-0 z-50 flex w-72 max-w-[85vw] flex-col bg-white shadow-2xl animate-in slide-in-from-left duration-200">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon size={16} className={active ? "text-[#005A36]" : "text-slate-400"} />
            ...
            <div className="border-t border-slate-100 p-3">
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-rose-50 px-4 py-2.5 text-xs font-bold text-rose-700 hover:bg-rose-100"
              >
                <LogOut size={14} />
                <span>Đăng xuất</span>
              </button>
      ...
      <div className={`flex min-h-screen flex-1 flex-col ${collapsed ? "md:ml-[72px]" : "md:ml-64"}`}>
        {/* Top Header Bar */}
        <header className="sticky top-0 z-40 flex h-14 sm:h-16 items-center justify-between border-b border-slate-200/80 bg-white/95 backdrop-blur-sm px-3 sm:px-5">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-slate-700 md:text-slate-500">
              <Shield size={14} className="text-[#005A36] shrink-0" />
              <span className="uppercase tracking-[0.08em] text-[11px] sm:text-xs line-clamp-1">Quản trị Hệ thống</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              href="/portal"
              className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-2.5 sm:px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-white hover:text-[#005A36] hover:border-emerald-200 transition-all active:scale-[0.97]"
            >
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 sm:gap-2 rounded-xl border border-slate-200 bg-white px-2 sm:px-2.5 py-1.5 hover:bg-slate-50 transition-all active:scale-[0.97]"
              >
        ...
        {/* Footer */}
        <footer className="border-t border-slate-200/80 bg-white/80 px-4 sm:px-6 py-3 text-center text-[10px] sm:text-[11px] text-slate-400">
