...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { poCode, images, man, machine, material, method, measurement, environment, rootCause } =
    await req.json();
  const effectivePoCode = (poCode || issue.poCode || "PO-DEFAULT").trim();
  if (!effectivePoCode || !man || !machine || !material || !method || !measurement || !environment || !rootCause) {
    return NextResponse.json(
