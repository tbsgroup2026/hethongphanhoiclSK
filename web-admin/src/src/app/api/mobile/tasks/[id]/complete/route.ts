...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!repairDetail) {
    return NextResponse.json({ error: "Vui lòng mô tả chi tiết công việc đã thực hiện" }, { status: 400 });
  }
  if (!Array.isArray(imagesBefore) || imagesBefore.length === 0) {
    return NextResponse.json({ error: "Cần ít nhất 1 ảnh trước khi xử lý" }, { status: 400 });
  }
  if (!Array.isArray(imagesAfter) || imagesAfter.length === 0) {
    return NextResponse.json({ error: "Cần ít nhất 1 ảnh sau khi xử lý" }, { status: 400 });
  }
  ...
  await logAuditEvent(prisma, {
    ...
    newStatus: "DONE",
    note: `${userRoleName} ${payload.name} hoàn thành xử lý: ${repairDetail}`,
  });
