...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!["LINE_LEADER", "QA", "DEPARTMENT_HEAD", "DIRECTOR", "ADMIN"].includes(payload.role)) {
    return NextResponse.json({ error: "Chỉ Trưởng line, QA hoặc Quản đốc mới được xác nhận đóng sự cố" }, { status: 403 });
  }
  ...
  if (me?.areaId && task.issue.areaId && task.issue.areaId !== me.areaId) {
